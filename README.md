...
0. **Enrôlement du terminal** (une seule fois par appareil) : l'Admin
   ...
   Keystore/Keychain ; le site est fixé par l'enrôlement et n'est ensuite jamais
   choisi par l'agent. Lien « Réenrôler ce terminal » sur l'écran de connexion.
...
localement à partir de la liste du jour signée (`GET /api/agent/offline-list`,
...
- les validations hors ligne sont mises en file, persistées, puis confrontées au
  registre central au retour du réseau (`POST /api/agent/resync`) ; les écarts
  remontent en événement de sécurité.
...
Le contrat OpenAPI publié ne type aucune réponse et laisse trois points ouverts :
le schéma de la preuve de possession à l'enrôlement, les schémas de réponse des
endpoints agent, et la structure du QR signé. Les parseurs de `src/lib/api.ts`
acceptent donc plusieurs graphies et lèvent `ContractError` plutôt que de
présenter un refus d'accès à l'agent. Pour la preuve d'enrôlement, l'hypothèse
retenue est isolée dans `buildProofMessage` / `encodeProof`
(`src/features/auth/enrollment.store.ts`) : ce sont les deux seules fonctions à
